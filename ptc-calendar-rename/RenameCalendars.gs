 ...
 *   1) Services (+) -> Google Calendar API -> Add.  The identifier must stay "Calendar".
 *   2) Run renameAllCalendars() and accept the OAuth prompt.
 *
...
function renameAllCalendars() {
  ...
  for (var i = 0; i < CLASS_CALENDARS.length; i++) {

    var row = CLASS_CALENDARS[i];
...
function logResults(results) {
  ...
  var sheet = ss.getSheetByName("Rename Log") ||
              ss.insertSheet("Rename Log");
